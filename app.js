...
const LINE=18, TOP=78, LEFT=66, RIGHT=16;
const C4Y=TOP+LINE*5;
const LEDGER_HALF=15;       // 加線を音符の左右15pxまで
const NOTE_W=10, NOTE_H=6.5;
let mode="1:2", selected=null, drag=false;

let cantus=[0,1,3,2,1,0].map(step=>({step,rest:false}));
let counter=[];
function resetCounter(){
  counter=(mode==="1:2"?[4,5,5,6,7,5,4,3,3,2,1,7]:[4,5,6,5,4,3]).map(step=>({step,rest:false}));
}
resetCounter();
...
function noteName(step){
  const N=["C","D","E","F","G","A","B"],o=Math.floor(step/7),i=((step%7)+7)%7;
  return N[i]+(4+o)
}
function midi(step){
  const B=[60,62,64,65,67,69,71],o=Math.floor(step/7),i=((step%7)+7)%7;
  return B[i]+o*12
}
...
function clef(ctx){
  ctx.save();
  ctx.font="78px 'Times New Roman',serif";
  ctx.textAlign="center";
  ctx.textBaseline="middle";
  ctx.fillStyle="#111";
  ctx.fillText("𝄞",LEFT-27,TOP+LINE*2.15);
  ctx.restore();
}

function whole(ctx,px,py){
  ctx.save();
  ctx.translate(px,py);
  ctx.rotate(-.28);
  ctx.strokeStyle="#111";
  ctx.lineWidth=2;
  ctx.beginPath();
  ctx.ellipse(0,0,NOTE_W,NOTE_H,0,0,Math.PI*2);
  ctx.stroke();
  ctx.restore();
}
function half(ctx,px,py){
  whole(ctx,px,py);
  ctx.fillStyle="#111";
  ctx.fillRect(px+7,py-34,1.6,34);
}

// 加線は「必要な線だけ」。音符を中央で貫く位置にも正確に置く。
function ledgerLines(ctx, px, step){
  const bottomLineStep = 2;  // 五線最下線 E4
  const topLineStep = 10;    // 五線最上線 F5
  ctx.save();
  ctx.strokeStyle="#111";
  ctx.lineWidth=1.35;
  ctx.lineCap="butt";
  ...
  if(step < bottomLineStep){
    // 下加線は C4(step 0), A3(-2), F3(-4)... の偶数stepに入る
    for(let s=0; s>=step; s-=2){
      if(s < bottomLineStep){
        const ly=y(s);
        ctx.beginPath();
        ctx.moveTo(px-LEDGER_HALF,ly);
        ctx.lineTo(px+LEDGER_HALF,ly);
        ctx.stroke();
      }
    }
  }
  ...
  if(step > topLineStep){
    // 上加線は A5(step 12), C6(14), E6(16)... に入る
...
function draw(ctx,c,notes,slots,type,editable){
  ...
  ctx.save();ctx.globalAlpha=.28;ctx.strokeStyle="#666";
  for(let i=0;i<=m;i++){
    const xx=LEFT+(w-LEFT-RIGHT)*i/m;
  ...
  notes.forEach((n,i)=>{
    ...
    if(n.rest){
      ctx.font="24px serif";ctx.fillStyle="#111";
      ctx.fillText("𝄽",xx-7,TOP+LINE*2+7);return
    }
    ...
    if(editable&&selected===i){
      ...
      ctx.beginPath();ctx.arc(xx,yy,17,0,Math.PI*2);ctx.stroke();
...
function redraw(){
  ...
  $("selectedPitch").textContent=selected==null?"未選択":counter[selected].rest?"休符":noteName(counter[selected].step)
}
...
cpC.addEventListener("pointerdown",e=>{
  drag=true;const p=point(e,cpC);
  selected=idx(p.x,counter.length,cpC.clientWidth);
  counter[selected].step=stepFromY(p.y);
  counter[selected].rest=false;redraw();e.preventDefault()
});
...
cpC.addEventListener("pointerup",()=>drag=false);
cpC.addEventListener("pointercancel",()=>drag=false);
...
function setMode(m){
  mode=m;selected=null;resetCounter();
...
$("mode12Btn").onclick=()=>setMode("1:2");
$("mode11Btn").onclick=()=>setMode("1:1");
$("resetBtn").onclick=()=>{resetCounter();selected=null;redraw()};
...
function analyze(){
  ...
  if(mode==="1:1"){
    for(let i=0;i<cantus.length;i++){
      ...
      const s=Math.abs(midi(counter[i].step)-midi(cantus[i].step))%12;
      const ok=[0,3,4,7,8,9].includes(s);
      out.push({sev:ok?"good":"error",title:"和声音程",loc:`第${i+1}小節`,msg:ok?"基本的な協和音程として扱えます。":"音程を確認してください。"})
    }
  }else{
    for(let m=0;m<cantus.length;m++){
      const cp=counter[m*2];if(cp.rest)continue;
      const s=Math.abs(midi(cp.step)-midi(cantus[m].step))%12;
      const ok=[0,3,4,7,8,9].includes(s);
      out.push({sev:ok?"good":"error",title:"強拍の和声音程",loc:`第${m+1}小節・強拍`,msg:ok?"強拍の基本和声音程として扱えます。":"強拍の音程を確認してください。"})
    }
  }
  return out
}
$("analyzeBtn").onclick=()=>{
  ...
  a.forEach(v=>{
    ...
    d.innerHTML=`<b>${v.title}</b><div class="muted">${v.loc}</div><div>${v.msg}</div>`;
    r.appendChild(d)
  });
  $("summaryBadge").textContent=`${a.length}件`
};
